...
def initializeGuardian(name, startDate):
    ...
    logging.debug("Initialize Guardian " + name + " Starting at Date " + str(startDate))
    ...
    cursor.execute('CREATE TABLE IF NOT EXISTS Leaderboard (Name TEXT, MembershipId TEXT, MembershipType INT, Character1Id TEXT, Character2Id TEXT, Character3Id TEXT, LastPlayed TEXT, SniperKills INT, SniperHeadShots INT, HeadshotPercentage REAL, MatchesPlayed INT)')
    ...
    for index, entry in enumerate(activityData):
        for slot, weapon in enumerate(entry[1]):
            try:
                ...
                logging.debug("Failed to get weapon info from weapon id " + str(weapon.referenceId) + " in match " + entry[0].instance)
    ...
    games =  cursor.execute("SELECT COUNT(Instance) FROM {} WHERE WeaponTypeName='Pulse Rifle'".format(user.globalDisplayName + 'GameData')).fetchone()[0]

    cursor.execute('INSERT INTO Leaderboard VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', (user.globalDisplayName, user.membershipId, user.membershipType, user.characterIds[0], user.characterIds[1], user.characterIds[2], user.dateLastPlayed, kills, headshots, headshotPercentage, games))
